...
namespace MyApp.Manage
{
    public class MysqlSchemaManage : ManageBase, ISchemaManage
    {
        ...
        public MysqlSchemaManage(string databaseName)
        {
        ...
        private async Task<IDbConnection> GetDbConnection(string dbName)
        {
            return await Task.FromResult(
                string.IsNullOrEmpty(dbName) || dbName == DatabaseName ||
                dbName == $"{DatabaseName}_test"
                    ? DbFactory.OpenDbConnection()
                    : DbFactory.OpenDbConnection(dbName));
        }

        private async Task<string> GetDatabaseName(string dbName)
        {
            return await Task.FromResult(string.IsNullOrEmpty(dbName) || dbName == DatabaseName
                ? DatabaseName
                : dbName);
        }

        private static async Task<string> GetIndexName(string tableName, string fieldName)
        {
            return await Task.FromResult($"idx_{tableName}_{fieldName}");
        }
        ...
        private async Task<string> GetColumnDefinition(ColumnInfo columnInfo)
        {
            ...
            switch (columnInfo.ColumnDataType)
            {
                ...
                case ColumnDataType.Float:
                    columnDef = $"double({columnInfo.FieldLength}, {columnInfo.Precision})";
                    break;
                case ColumnDataType.String:
                    columnDef = columnInfo.FieldLength.HasValue
                        ? $"varchar({columnInfo.FieldLength})"
                        : "text";
                    break;
                default:
